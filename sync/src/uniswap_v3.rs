...
use ethers::types::{H160, H256, U256, I256, U64};
...
impl UniswapV3Metadata {
	pub fn calculate_virtual_reserves(&self) -> (u128, u128) {
		let price = BigFloat::from_u128(
			(U256::from(self.sqrt_price.overflowing_mul(self.sqrt_price).0) >> 128).as_u128(),
		)
	...
	pub async fn get_word(
		...
		if block_number.is_some() {
			...
			Ok(crate::abi::UniswapV3Pool::new(H160::from_str(&self.address)?, middleware.clone())
	...
	pub async fn get_tick_info(
		...
		let v3_pool = crate::abi::UniswapV3Pool::new(H160::from_str(&self.address)?, middleware.clone());
	...
	pub async fn simulate_swap(
		...
		let mut current_state = CurrentState {
			sqrt_price_x_96: U256::from(self.sqrt_price), //Active price on the pool
		...
		tokio::spawn(async move {
			...
			for pair in pairs.read().await.iter() {
				...
				let pool = Pool {
					...
					y_amount: 0,
					x_to_y: true,
					provider: LiquidityProviders::UniswapV3(Default::default())
				};
				let mut w = pools.write().await;
				w.insert(pool.address.clone(), pool);
			}
			println!("{:?} Pools: {}",LiquidityProviders::UniswapV3(Default::default()), pools.read().await.len());
		})
	}
